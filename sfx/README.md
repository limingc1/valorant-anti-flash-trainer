...
这个目录里的 `mp3` / `wav` / `ogg` / `m4a` **会跟着 git 一起提交**，push 之后
Cloudflare Pages 自动部署，线上也能听到（不再是只有本地生效的合成音）。

自己做着玩的项目所以这样最省事。两点注意：

- **文件尽量短、尽量压缩过**。git 里每个版本的二进制都会永久留在历史里，
  一次性放几十 MB 的 wav 会让仓库越来越沉。短音效用 mp3，单个几十 KB 就够。
- 别放整段录屏/长录音进来，切好再放。
...
这个目录一开始是空的。下面几条路任选，**已经有现成的单个音频文件就直接跳到「命名规则」**
—— 只要是 mp3/wav/ogg/m4a、且一个文件就是一个音，改好名字丢进来即可，不需要任何转换工具。
